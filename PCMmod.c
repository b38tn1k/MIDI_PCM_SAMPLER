...
ISR(TIMER1_COMPA_vect) {
  if (sample >= sounddata_length) {
    ...
    if (bitcrush){
      if (bit_crush_flag) {
        OCR2A = pgm_read_byte(&sounddata_data[sample + 10]); //this one works
        bit_crush_flag = false;
        --sample;
      } else {
        OCR2A = pgm_read_byte(&sounddata_data[sample]); //this one works
        bit_crush_flag = true;
      }
    } else {
      // OCR2A = pgm_read_byte((glitch_mix * (uint16_t)&sounddata_data[sample] + (100-glitch_mix) * (uint16_t)mixsounddata_data2[sample]) /100);
      OCR2A = pgm_read_byte(&sounddata_data[sample]); //this one works
    }
...
void startGlitchPlayback(unsigned char const *data, int length, float pitch_glitch, bool bit_crush)
{
  sounddata_data = data;
  ...
  OCR1A = F_CPU / abs(SAMPLE_RATE*pitch_glitch);    // 16e6 / 8000 = 2000
